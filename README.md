...
### 10. 🕒 VOB 변경 이력 & 파일 버전 좌우 비교 (VOB History & Version Compare)
> CR 번호와 무관하게 **VOB(ClearCase 최상위 소스 저장소) 단위**로 코드 변경 흐름을 시간순으로 탐색합니다. 왼쪽에서 VOB를 검색·선택하면 관련 CR과 수집 현황이 요약되고, 파일 항목을 펼치면 별도 버튼 없이 **해당 파일의 전체 버전 이력(0~최신)이 좌우 스크롤 비교 화면으로 즉시** 표시됩니다. 각 버전 컬럼 상단에는 해당 버전을 만든 CR 번호가 함께 표시되어, 어떤 CR이 무엇을 바꿨는지 한눈에 추적할 수 있습니다.

![VOB 변경 이력 및 버전 좌우 비교](docs/images/11_vob_history_version_compare.png)
...
| **🕒 VOB 변경 이력 & 버전 좌우 비교** | CR 번호와 무관하게 VOB 단위로 변경 흐름을 시간순 탐색. 파일을 펼치면 버튼 없이 바로 전체 버전(0~최신)이 좌우 스크롤 비교 화면으로 표시되고, 컬럼마다 해당 버전을 만든 CR 번호가 함께 보임 |
...
### v1.0.5
* 🔧 **다중 버전 SSH 조회 시 커넥션 풀 고갈로 인한 타임아웃 수정**: 파일 하나의 버전을 여러 개(예: 5개) 한번에 조회할 때 필요한 커넥션 수를 한꺼번에 요청해 풀 최대치(호스트당 3개)를 초과 → 자신이 점유한 커넥션 반환을 기다리며 15초 타임아웃으로 실패하던 문제 해결. 이제 풀 용량만큼 배치로 나눠 순차 조회하고, 각 커넥션을 배치 전체가 아닌 자기 작업 완료 즉시 반환

### v1.0.4
* 🎨 **VOB 변경 이력 - 버전 비교 화면을 좌우 스크롤 비교 뷰로 전면 개편**: 파일 항목을 펼치면 읽기 어려운 unified diff 텍스트 대신, 전체 버전 이력(0~최신)을 좌우 스크롤로 나란히 비교하는 화면이 별도 버튼 없이 펼치는 즉시 표시됨 (ClearCase에서 전체 버전을 병렬 조회)

### v1.0.3
* 🆕 **VOB 변경 이력 메뉴 신설**: CR 번호와 무관하게 VOB(ClearCase 최상위 소스 저장소) 단위로 코드 변경 흐름을 시간순으로 탐색. VOB 목록/검색, 캐시된 CR 즉시 표시, 미수집 CR은 "아직 수집 안됨" 배지와 우선 수집 요청 지원
* 🆕 **파일 버전 다중 비교(3-4-7-8 형태)**: 한 CR에서 같은 파일을 여러 번 체크인한 경우, 해당 파일의 모든 버전을 좌우 스크롤로 한 화면에서 비교하는 `버전 N개 한번에 비교` 기능 추가
* 🔧 **파일-버전 매칭 로직 수정**: 서로 다른 VOB에 동일한 이름의 파일이 있을 때(예: `CsbUtil.c`) 체크인 로그 매칭이 파일명만으로 비교되어 엉뚱한 CR의 버전 이력이 섞여 들어가던 문제 해결 (VOB 전체 경로 기준으로 정밀 매칭)
